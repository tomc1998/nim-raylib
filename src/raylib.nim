...
#       [text] stb_truetype (Sean {.bycopy.} Barret) for ttf fonts loading
...
type Quaternion* {.bycopy.} = Vector4
# Matrix type (OpenGL {.bycopy.} style 4x4 - right handed, column major)
...
type Texture* {.bycopy.} = Texture2D
# TextureCubemap type, actually, same as Texture2D
type TextureCubemap* {.bycopy.} = Texture2D
# RenderTexture2D type, for texture rendering
type RenderTexture2D* {.bycopy.} = object
...
type RenderTexture* {.bycopy.} = RenderTexture2D
# N-Patch layout info
...
# Ray type (useful {.bycopy.} for raycast)
...
# Music stream type (audio {.bycopy.} file streaming from memory)
...
# Android buttons
type AndroidButton* {.bycopy.} = enum 
    KEY_BACK            = 4 
    KEY_VOLUME_UP       = 24 
    KEY_VOLUME_DOWN     = 25 
    KEY_MENU            = 82 
...
# Gamepad Buttons
type GamepadButton* {.bycopy.} = enum 
...
type ShaderLocationIndex* {.bycopy.} = enum 
...
type ShaderUniformDataType* {.bycopy.} = enum 
    UNIFORM_FLOAT = 0 
    UNIFORM_VEC2 
    UNIFORM_VEC3 
    UNIFORM_VEC4 
...
# Material map type
type MaterialMapType* {.bycopy.} = enum 
...
type TextureFilterMode* {.bycopy.} = enum 
...
# Cubemap layout type
type CubemapLayoutType* {.bycopy.} = enum 
...
type TextureWrapMode* {.bycopy.} = enum 
...
proc IsWindowMinimized*(): bool {.RLAPI, importc: "IsWindowMinimized".} # Check if window has been minimized
proc IsWindowFocused*(): bool {.RLAPI, importc: "IsWindowFocused".} # Check if window has been focused
...
proc GetMonitorRefreshRate*(monitor: int32): int32 {.RLAPI, importc: "GetMonitorRefreshRate".} # Get primary monitor refresh rate
...
proc GetImagePalette*(image: Image; maxPaletteSize: int32; extractCount: pointer): ptr Color {.RLAPI, importc: "GetImagePalette".} # Get color palette from image to maximum size (memory should be freed)
...
proc DrawTextureTiled*(texture: Texture2D; sourceRec: Rectangle; destRec: Rectangle; origin: Vector2; rotation: float32; scale: float32; tint: Color) {.RLAPI, importc: "DrawTextureTiled".} # Draw part of a texture (defined by a rectangle) with rotation and scale tiled into destRec.
...
proc Fade*(color: Color; alpha: float32): Color {.RLAPI, importc: "Fade".} # Returns color with alpha applied, alpha goes from 0.0f to 1.0f
...
proc ColorAlpha*(color: Color; alpha: float32): Color {.RLAPI, importc: "ColorAlpha".} # Returns color with alpha applied, alpha goes from 0.0f to 1.0f
proc ColorAlphaBlend*(dst: Color; src: Color; tint: Color): Color {.RLAPI, importc: "ColorAlphaBlend".} # Returns src alpha-blended into dst color with tint
...
proc SetMaterialTexture*(material: ptr Material; mapType: int32; texture: Texture2D) {.RLAPI, importc: "SetMaterialTexture".} # Set texture for a material map type (MAP_DIFFUSE, {.bycopy.} MAP_SPECULAR...)
